...
import { Keypair, PublicKey, Connection, SYSVAR_RENT_PUBKEY, SystemProgram, ComputeBudgetProgram } from "@solana/web3.js";
import { TOKEN_PROGRAM_ID } from "@solana/spl-token";
import * as spl from "@solana/spl-token";
import { Metadata, PROGRAM_ID as METADATA_PROGRAM_ID } from "@metaplex-foundation/mpl-token-metadata";
import { Metaplex } from "@metaplex-foundation/js";
import { PublicKeyMap } from "@hubbleprotocol/kamino-lending-sdk";
...
export class MathsolClient {
    ...
    async luckyBoxInitialize(admin: Keypair, signer: PublicKey, mintStartTime: BN, swapStartTime: BN, seedNftCount: BN) {
        ...
        console.log("luckyBoxInitialize start ..", MathsolClient.nft_price, new PublicKey(MathsolClient.nft_taxer));
        const transaction = await this.program.methods
            .luckyBoxInitialize(signer, mintStartTime, swapStartTime, seedNftCount, 
                MathsolClient.nft_price, new PublicKey(MathsolClient.nft_taxer))
            .accounts({
    ...
    async luckyBoxUpdate(admin: Keypair, signer: PublicKey, mintStartTime: BN, swapStartTime: BN, seedNftCount: BN) {
        const transaction = await this.program.methods
            .luckyBoxUpdate(signer, mintStartTime, swapStartTime, seedNftCount, 
                MathsolClient.nft_price, 
                new PublicKey(MathsolClient.nft_taxer))
            .accounts({
                admin: admin.publicKey,
                luckyBoxAccount: this.findLuckyBoxAccountPDA(),
                systemProgram: SystemProgram.programId,
    ...
    async luckyBoxMintNft(user: Keypair, referrer: PublicKey) {
        ...
        const method = this.program.methods.luckyBoxMintNft().accounts({
            ...
            taxerIn: new PublicKey(MathsolClient.nft_taxer),
            collectionMint: collectionPDA,
    ...
    async fairLaunchInitialize(admin: Keypair, signer: PublicKey, startTime: BN, drawPrice: BN, solRefundAmount: BN, tokenClaimAmount: BN) {
        console.log('fairLaunchInitialize start')
        const method = await this.program.methods.
        fairLaunchInitialize(
            signer, startTime, drawPrice, solRefundAmount, tokenClaimAmount, 
            MathsolClient.manager_address_rate,
            MathsolClient.team_address_rate,
            MathsolClient.pool_address_rate,
            new PublicKey(MathsolClient.manager_address), 
            new PublicKey(MathsolClient.team_address), 
            new PublicKey(MathsolClient.pool_address)
        )
        .accounts({
            admin: admin.publicKey,
    ...
    async fairLaunchUpdate(admin: Keypair, signer: PublicKey, startTime: BN, drawPrice: BN, solRefundAmount: BN, tokenClaimAmount: BN) {
        const method = await this.program.methods.fairLaunchUpdate(signer, startTime, drawPrice, solRefundAmount, tokenClaimAmount, 
            MathsolClient.manager_address_rate, MathsolClient.team_address_rate, MathsolClient.pool_address_rate,
            new PublicKey(MathsolClient.manager_address), new PublicKey(MathsolClient.team_address), new PublicKey(MathsolClient.pool_address)
        ).accounts({
            admin: admin.publicKey,
            fairLaunchAccount: this.findFairLaunchAccountPDA(),
            systemProgram: SystemProgram.programId,
    ...
    async fairLaunchDraw(user: Keypair) {
        ...
        const transaction = await this.program.methods
            ...
            .accounts({
                ...
                managerIn: new PublicKey(MathsolClient.manager_address),
                teamIn: new PublicKey(MathsolClient.team_address),
                poolIn: new PublicKey(MathsolClient.pool_address),
                fairLaunchAccount: this.findFairLaunchAccountPDA(),
        ...
        const addPriorityFee = ComputeBudgetProgram.setComputeUnitPrice({
            microLamports:40_0000
        });
        const modifyComputeUnits = anchor.web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 40_0000 });
        const t = new anchor.web3.Transaction().add(...transactions, addPriorityFee, modifyComputeUnits);
        return t
    }
    ...
    async fairLaunchRefund(user: Keypair, signer: PublicKey, drawId: BN, message: Uint8Array, signature: Uint8Array) {
        ...
        const modifyComputeUnits = anchor.web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 40_0000 });
        const t = new anchor.web3.Transaction().add(ed25519Instruction, addPriorityFee, modifyComputeUnits, await method.transaction());
        return t
    }
    ...
    async fairLaunchClaim(user: Keypair, signer: PublicKey, drawId: BN, message: Uint8Array, signature: Uint8Array) {
        ...
        const modifyComputeUnits = anchor.web3.ComputeBudgetProgram.setComputeUnitLimit({ units: 40_0000 });
        const t = new anchor.web3.Transaction().add(ed25519Instruction, modifyComputeUnits, addPriorityFee,await method.transaction());
        return t
    }
